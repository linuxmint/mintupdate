...
######### TREEVIEW/SELECTION FUNCTIONS #######
    ...
    def treeview_right_clicked(self, widget, event):
        if event.button == 3:
            ...
            if (iter != None):
                package_update = model.get_value(iter, UPDATE_OBJ)
                package_version = model.get_value(iter, UPDATE_NEW_VERSION)
                menu = Gtk.Menu()
                ...
                menuItem.connect("activate", self.add_to_ignore_list, "%s=%s" % (package_update.source_name, package_version))
                menu.append(menuItem)
